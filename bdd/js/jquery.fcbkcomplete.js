...
(function( $, undefined ) {
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function elPrepare() {
        ...
        $.each(element.children('option'), function(i, option) {
          ...
          if (option.hasClass("selected")) {
            var id = addItem(option.text(), option.val(), true, option.hasClass("locked"), 1);
      ...
      function addItem(title, value, preadded, locked, focusme, add) {
        if (!maxItems()) {
          return false;
        }
        var liclass = "bit-box" + (locked ? " locked": "");
        var id = randomId();
        var quebra = xssDisplay(title).wordWrap(24,'<br/>', true);
        var txt = quebra;
