<!-- Set names -->
<?php $field = 'Species name'; ?>
<?php $field_plural = 'Species names'; ?>

<script>
    $(document).ready(bootSpeciesName)

    function bootSpeciesName() {

        <?php foreach($listName as $n=>$ar): ?>
            insertLine("<?php echo $ar->idspeciesname;?>", "<?php echo $ar->speciesname; ?>");
        <?php endforeach; ?>
    ...
    function insertLine(id, field)
    {
        ...
        $('#lines').append(line);if(field.length > 0){
        	$('#lines').append(line);
        }
    }
