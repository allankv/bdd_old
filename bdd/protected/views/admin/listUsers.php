...
<script>
    ...
    function filter(senderValue){
        ...
        $.ajax({
        	type:'POST',
            url:'index.php?r=admin/FilterUsers',
    ...
    function insertLine(rs){
        var line ='<tr id="id__ID_"><td style="text-indent:5px;">_NAME_</td><td style="text-indent:5px;">_GROUP_</td><td style="text-indent:5px;">_EMAIL_</td><td style="width:160px; text-align:center; text-indent:0px;">_BUTTONS_</td></tr>';
        var aux = line;

                
        var btnUpdate = "<div class='btnUpdate'><a href='index.php?r=admin/goToMaintainUser&id="+rs.id+"'><ul class='iconJQueryHover iconJQuery ui-widget ui-helper-clearfix'><li class='ui-state-default ui-corner-all' title='Update Specimen Record'><span class='ui-icon ui-icon-pencil'></span></li></ul></a></div>";
        var btnDelete = "<div class='btnDelete'><a href='javascript:deleteUserRecord("+rs.id+");'><ul class='iconJQueryHover iconJQuery ui-widget ui-helper-clearfix'><li class='ui-state-default ui-corner-all' title='Delete Specimen Record'><span class='ui-icon ui-icon-trash'></span></li></ul></a></div><div style='clear:both'></div>";
        
		//console.log(rs);

        aux = aux.replace('_ID_',rs.id);
        
        aux = aux.replace('_NAME_',rs.username);

        aux = aux.replace('_EMAIL_',rs.email);

        aux = aux.replace('_GROUP_',rs.group);
    ...
    function deleteUserRecord (idgroup)
    {
...
<h1 style="padding-left:10px; float:left;"><?php echo Yii::t('yii', 'List users records'); ?></h1>
...
<div id="rs" class="item">
    <table id="tablelist" class="list">
        <thead><tr><th style="text-align:left;">Username</th><th style="text-align:left;">Group</th><th style="text-align:left;">E-mail</th><th>Options</th></tr>
        </thead>
...
function configFilter(id,field,controller,_filterList){
    ...
    $(field).catcomplete({
        source: 'index.php?r='+controller+'/UserSearch',
        minLength: 2,
...
function deleteRecord (id, controller) {
    $.ajax({
        type:'POST',
        url:'index.php?r='+controller+'/DeleteUser',
        data: {
            "id":id
        },
        success:function(data){
            if (data==1){
				alert('Successfully deleted user record');
            }
            else if (data==-1){
            	alert("You can't delete this user because there is/are specimens associated to this user.");
            }
        }
    });
}
