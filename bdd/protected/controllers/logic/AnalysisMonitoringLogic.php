...
include_once 'RecordLevelElementLogic.php';
include_once 'OccurrenceElementLogic.php';
include_once 'TaxonomicElementLogic.php';
include_once 'LocalityElementLogic.php';
include_once 'GeospatialElementLogic.php';

include_once 'DenominationLogic.php';
include_once 'CultureLogic.php';
include_once 'TechnicalCollectionLogic.php';

class AnalysisMonitoringLogic {
	public function filter($filter) {
        ...
        $c['select'] = 'SELECT r.isrestricted, mon.idmonitoring, scn.scientificname, o.catalognumber, inst.institutioncode, coll.collectioncode, den.denomination, cul.culture, tech.technicalcollection ';
        ...
        $c['where'] = ' WHERE 1 = 1 '.$institutionCodeWhere.$collectionCodeWhere.$scientificNameWhere.$basisOfRecordWhere.$catalogNumberWhere.
                ...
                $countryWhere.$stateProvinceWhere.$countyWhere.$municipalityWhere.$localityWhere.$denominationWhere.$cultureWhere.$technicalCollectionWhere;
    ...
    public function time($filter) {
        ...
        // Para Monitoring
        $c['join'] = $c['join'].' LEFT JOIN monitoring mon ON log.id = mon.idmonitoring ';
        $c['join'] = $c['join'].' LEFT JOIN denomination den ON mon.iddenomination = den.iddenomination  ';
        $c['join'] = $c['join'].' LEFT JOIN culture cul ON mon.idculture = cul.idculture  ';
        $c['join'] = $c['join'].' LEFT JOIN technicalcollection tech ON mon.idtechnicalcollection = tech.idtechnicalcollection  ';

        $c['select'] = 'select date, type, module, count(*) ';
        $c['where'] = " where type = 'create' and module = 'monitoring' ".$denominationWhere.$cultureWhere.$technicalCollectionWhere;
        ...
        $sql = $c['select'].$c['from'].$c['join'].$c['where'].$c['group'].$c['orderby'];
        $createMON = WebbeeController::executaSQL($sql);
        
        $c['select'] = 'select date, type, module, count(*) ';
        $c['where'] = " where type = 'update' and module = 'monitoring' ".$denominationWhere.$cultureWhere.$technicalCollectionWhere;
        $sql = $c['select'].$c['from'].$c['join'].$c['where'].$c['group'].$c['orderby'];
        $updateMON = WebbeeController::executaSQL($sql);
        
        $c['select'] = 'select date, type, module, count(*) ';
        $c['where'] = " where type = 'delete' and module = 'monitoring' ".$denominationWhere.$cultureWhere.$technicalCollectionWhere;
        ...
        $sql = $c['select'].$c['from'].$c['join'].$c['where'].$c['group'].$c['orderby'];
        $deleteMON = WebbeeController::executaSQL($sql);
        
        // Para o restante
        $c['select'] = 'select date, type, module, count(*) ';
        $c['where'] = " where type = 'create' and module <> 'specimen' and module <> 'interaction' and module <> 'monitoring' ";
        ...
        $c['select'] = 'select date, type, module, count(*) ';
        $c['where'] = " where type = 'update' and module <> 'specimen' and module <> 'interaction' and module <> 'monitoring' ";
        $sql = $c['select'].$c['from'].$c['where'].$c['group'].$c['orderby'];
        $update = WebbeeController::executaSQL($sql);
        
        $c['select'] = 'select date, type, module, count(*) ';
        $c['where'] = " where type = 'delete' and module <> 'specimen' and module <> 'interaction' and module <> 'monitoring' ";
        ...
        $create = array_merge($create, $createSP, $createINT, $createMON);
        $update = array_merge($update, $updateSP, $updateINT, $updateMON);
        $delete = array_merge($delete, $deleteSP, $deleteINT, $deleteMON);
    	...
    	// Acumula registros (o número de registros em uma data depende das datas anteriores)
    	$numrecords = array();
