...
class AnalysisPDF extends TCPDF {
	// Page footer
...
class BasisOfRecordPDF extends AnalysisPDF {
	//Page header
	public function Header() {
		...
		$this->SetFont('helvetica', '', 10);
		$this->Cell(0, 2, '  Analysis Tools - Basis of Records', 0, 1, 'L', false, '', 0, false, 'T', 'M');
		// Line
...
class InstitutionCodePDF extends AnalysisPDF {
	//Page header
	public function Header() {
		// Logo
		$image_file = 'images/main/logo_bdd.jpg';
		...
		$this->SetTextColor(0, 0, 0);
		$this->SetFont('helvetica', '', 10);
		$this->Cell(0, 2, '  Analysis Tools - Institution Code', 0, 1, 'L', false, '', 0, false, 'T', 'M');
		// Line
		$this->SetFont('helvetica', '', 5);
		$this->Cell(0, 1, '', 'B', 1, 'L', false, '', 0, false, 'T', 'M');
	}
...
class CollectionCodePDF extends AnalysisPDF {
	//Page header
	public function Header() {
		// Logo
		$image_file = 'images/main/logo_bdd.jpg';
		...
		$this->SetTextColor(0, 0, 0);
		$this->SetFont('helvetica', '', 10);
		$this->Cell(0, 2, '  Analysis Tools - Collection Code', 0, 1, 'L', false, '', 0, false, 'T', 'M');
		// Line
		$this->SetFont('helvetica', '', 5);
		$this->Cell(0, 1, '', 'B', 1, 'L', false, '', 0, false, 'T', 'M');
	}
...
class DataQualityPDF extends AnalysisPDF {
	//Page header
	public function Header() {
		// Logo
		$image_file = 'images/main/logo_bdd.jpg';
		...
		$this->SetTextColor(0, 0, 0);
		$this->SetFont('helvetica', '', 10);
		$this->Cell(0, 2, '  Analysis Tools - Data Quality', 0, 1, 'L', false, '', 0, false, 'T', 'M');
		// Line
		$this->SetFont('helvetica', '', 5);
		$this->Cell(0, 1, '', 'B', 1, 'L', false, '', 0, false, 'T', 'M');
	}
...
class AnalysisPrint extends Printer {
	...
	public function printAnalysis($rs, $lowercase, $colors) {
		...
		$pdf->SetX(0);
		$pdf->Cell($pdf->getPageWidth(), 0, $uppercase . ' versus Number of Specimen Occurrence Records', '', 1, 'C', true, '', 0, false, 'T', 'M');
		...
		$header = array('', $uppercase, 'Number of Records', 'Percents');
		//Data loading for the table
		$txt = "tmp/bdd-analysistools-" . $lowercase . "-data.txt";
		$file = fopen($txt,"w");
		...
		$file = "tmp/bdd-".$lowercase."-".time().".pdf";
		$pdf->Output($file, 'F');
	...
	public function printDataQuality($rs) {
		// create new PDF document
		$pdf = new DataQualityPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		$uppercase = 'Data Quality';
		$lowercase = 'dataquality';
		// set document information
		$pdf->SetCreator(PDF_CREATOR);
		$pdf->SetAuthor('Biodiversity Data Digitizer');
		$pdf->SetTitle("Analysis Tools - " . $uppercase . " versus Number of Specimen Occurrence Records");
		...
		// print chart image
		$pdf->Image('@'.$rs['chartDQ1'], -5, 30);
		$pdf->Image('@'.$rs['chartDQ2'], -5, 155);
		
		$pdf->addPage();
		
		$pdf->Image('@'.$rs['chartDQ3'], -5, 30);
		$pdf->Image('@'.$rs['chartDQ4'], -5, 155);
		
		$pdf->addPage();
		$pdf->Image('@'.$rs['chartDQ5'], -5, 30);
		
		//Close and output PDF document
		
		$file = "tmp/bdd-analysis".$lowercase."-".time().".pdf";
		$pdf->Output($file, 'F');
